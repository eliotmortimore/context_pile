...
import { Clipboard, FileDown, Loader2, Plus, Trash2, Play, CheckCircle2, XCircle, Code, Eye } from 'lucide-react';
...
export default function LocalDashboard() {
  ...
  const processDoc = async (doc: DocItem) => {
    ...
    try {
      const res = await fetch('/api/local-processor', {
        method: 'POST',
      ...
      <aside className="w-80 flex flex-col border-r border-zinc-800 bg-[#0f0f0f]">
        <div className="p-4 border-b border-zinc-800 flex justify-between items-center">
          <h1 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
            ...
            ContextPile <span className="text-xs text-emerald-500 font-normal">(Local)</span>
          </h1>
        </div>
        ...
        <div className="p-4 border-b border-zinc-800 space-y-3">
          <textarea
            className="w-full h-24 bg-zinc-900 border border-zinc-700 rounded-lg p-3 text-xs text-zinc-300 placeholder:text-zinc-600 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 outline-none resize-none"
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-medium py-2 rounded-lg transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {documents.map(doc => (
            <div
              ...
              className={`group flex items-center justify-between p-3 rounded-lg cursor-pointer transition-colors ${selectedDocId === doc.id
                  ? 'bg-zinc-800 border-zinc-700'
                  : 'hover:bg-zinc-900 border border-transparent'
                }`}
            >
              <div className="flex items-center gap-3 overflow-hidden">
                {doc.status === 'processing' && <Loader2 className="w-4 h-4 text-emerald-500 animate-spin shrink-0" />}
        ...
        {selectedDoc && selectedDoc.status === 'success' && selectedDoc.result ? (
            ...
            <header className="h-16 flex items-center justify-between px-6 border-b border-zinc-800 bg-[#0f0f0f]">
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-4 text-xs text-zinc-500">
                  ...
                  <a href={selectedDoc.url} target="_blank" rel="noopener" className="hover:text-emerald-400 truncate max-w-xs">{selectedDoc.url}</a>
                </div>
              </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-zinc-500 gap-4">
            {selectedDoc?.status === 'processing' ? (
              <div className="flex flex-col items-center gap-2"><Loader2 className="w-8 h-8 animate-spin text-emerald-500" /><p>Processing...</p></div>
